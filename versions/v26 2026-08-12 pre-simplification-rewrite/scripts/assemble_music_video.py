...
def _target_frames(manifest: dict[str, object]) -> tuple[int, int, int, Decimal]:
    ...
    frames = _ceil_frame_count(duration, fps_num, fps_den)
    return frames, fps_num, fps_den, duration


def _ceil_frame_count(duration: Decimal, fps_num: int, fps_den: int) -> int:
...
def _visual_path(output_path: Path) -> Path:
    return output_path.parent / "visual.mp4"
...
def _sha256(path: Path) -> str:
    ...
    try:
        ...
        raise AssemblyError(f"Could not read master audio for SHA-256: {path}") from error
    return digest.hexdigest()


def _verify_master_digest(manifest: dict[str, object]) -> None:
    master = manifest["master"]
    assert isinstance(master, dict)
    path = Path(str(master["path"]))
    expected = str(master["sha256"])
    if _sha256(path) != expected:
        raise AssemblyError("Master audio SHA-256 does not match the manifest")
...
def _verify_master_facts(
    ...
    path = Path(str(master["path"]))
    try:
        actual_sample_rate = int(stream["sample_rate"])
    except (KeyError, TypeError, ValueError) as error:
        raise AssemblyError("Could not read master audio sample rate") from error
    try:
        actual_channels = int(stream["channels"])
    except (KeyError, TypeError, ValueError) as error:
        raise AssemblyError("Could not read master audio channel count") from error
    if actual_sample_rate != int(master["sample_rate"]):
        raise AssemblyError("Master audio sample rate does not match the manifest")
    if actual_channels != int(master["channels"]):
        raise AssemblyError("Master audio channel count does not match the manifest")

    actual_duration = _media_duration(probe, stream, path)
    declared_duration = _decimal(master["decoded_duration_seconds"], "master duration")
    actual_target = _ceil_frame_count(actual_duration, fps_num, fps_den)
    declared_target = _ceil_frame_count(declared_duration, fps_num, fps_den)
    if actual_target != declared_target:
        raise AssemblyError(
            "Master audio coverage and duration metadata disagree with the manifest target frame count"
        )
    return actual_duration


def _probed_frame_rate(stream: dict[str, Any]) -> Fraction:
    value = stream.get("avg_frame_rate")
    if not isinstance(value, str):
        raise AssemblyError("Could not read final video frame rate")
    try:
        rate = Fraction(value)
    except (ValueError, ZeroDivisionError) as error:
        raise AssemblyError("Could not read final video frame rate") from error
    if rate <= 0:
        raise AssemblyError("Could not read final video frame rate")
    return rate
...
def assemble(
    ...
    _verify_master_digest(resolved)
    _require_tools()
    ...
    actual_master_duration = _verify_master_facts(
        resolved, master_probe, master_stream, fps_num, fps_den
    )
    ...
    candidate = work_directory / "candidate.mp4"
    visual = _visual_path(candidate)
    _run(
        _visual_command(resolved, candidate, target_frames, fps_num, fps_den),
    ...
    _run(build_ffmpeg_command(resolved, candidate), "Could not mux supplied master audio")
    final_probe, final_video, final_audio = _output_facts(candidate)
    try:
    ...
    actual_rate = _probed_frame_rate(final_video)
    expected_rate = Fraction(fps_num, fps_den)
    if actual_rate != expected_rate:
        raise AssemblyError(
            f"Final video frame rate {actual_rate} differs from manifest frame rate {expected_rate}"
        )
    ...
    try:
        ...
        raise AssemblyError(f"Output already exists: {output}") from error
    except OSError as error:
        raise AssemblyError(f"Could not publish output atomically: {output}") from error

    return {
        "output_path": str(output.resolve()),
        "work_directory": str(work_directory),
        "visual_intermediate_path": str(visual.resolve()),
        "mux_candidate_path": str(candidate.resolve()),
        ...
        "probed_frame_rate": {
            "fps_num": actual_rate.numerator,
            "fps_den": actual_rate.denominator,
        },
