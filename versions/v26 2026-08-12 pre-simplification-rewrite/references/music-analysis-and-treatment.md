...
When authorized lyrics are supplied, add `--lyrics "<lyrics-path>"`. The analyzer records the lyrics-file hash but does not emit the lyric body. It returns source codec metadata, decoded duration, sample rate, channels, EBU R128 integrated loudness, true peak, loudness range, energy windows, silence regions, onset candidates, tempo candidates, and confidence. A silent source uses JSON `null` for non-finite integrated loudness and true peak, with `status: below_measurement_floor`. Timing and tempo signals are candidates, not musical ground truth.
