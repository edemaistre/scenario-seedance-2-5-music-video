...
Represent edit boundaries as frames, not floating-point master time. Use decimal seconds only for source trims. Resolve creative cut choices before deterministic assembly.
...
8. refuses to overwrite an existing output or visual intermediate
...
- duration within one frame plus 20 milliseconds of the master
...
- an explicit non-bit-exact status for the single AAC transcode path
