...
def _validate_geometry(
    ...
    elif aspect_ratio in ASPECT_RATIOS and (width, height) != RESOLUTION_GEOMETRY[resolution][aspect_ratio]:
...
def _validate_shot(
    ...
    if frame_range is not None and source_span is not None and delivery is not None:
        ...
        if (
            ...
            and parameters["duration"] < required_span
        ):
...
def _validate_generation(
    ...
    if delivery is not None:
        ...
        if mode in {"text", "reference"} and parameters.get("aspectRatio") != aspect_ratio:
        ...
        if mode in {"first_frame", "first_last_frame", "edit", "extend"} and "aspectRatio" in parameters:
    ...
    if parameters.get("outputFormat") not in {"mp4", "mov"}:
    ...
    if mode in {"first_frame", "first_last_frame"} and _is_asset_id(parameters.get("image")):
