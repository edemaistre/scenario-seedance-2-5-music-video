...
def _probe_audio(path: Path) -> tuple[float, int, int]:
    ...
    if not math.isfinite(duration) or duration <= 0:
        raise ValueError(f"Master audio has an invalid duration: {path}")
...
def _energy_records(values: list[float], window_seconds: float) -> list[dict[str, float]]:
    return [
        {
            "start_seconds": round(index * window_seconds, 4),
            "end_seconds": round((index + 1) * window_seconds, 4),
            "rms": round(value, 6),
        }
...
def _silence_regions(values: list[float], window_seconds: float) -> list[dict[str, float]]:
            ...
            regions.append(
                {
                    ...
                    "end_seconds": round(index * window_seconds, 4),
                }
            )
            start_index = None
    if start_index is not None:
        regions.append(
            {
                "start_seconds": round(start_index * window_seconds, 4),
                "end_seconds": round(len(values) * window_seconds, 4),
            }
        )
    return regions
...
def _onset_candidates(values: list[float], window_seconds: float) -> list[dict[str, float]]:
    if not values:
        return []
    threshold = max(0.01, max(values) * 0.15)
    candidates: list[dict[str, float]] = []
    previous = 0.0
    for index, value in enumerate(values):
        strength = value - previous
...
def analyze_audio(path: Path, lyrics_path: Path | None = None) -> dict[str, object]:
    ...
    duration_seconds, sample_rate, channels = _probe_audio(master_path)
    ...
    return {
        ...
        "analysis": {
            "energy_windows": _energy_records(energy_values, ENERGY_WINDOW_SECONDS),
            "silence_regions": _silence_regions(energy_values, ENERGY_WINDOW_SECONDS),
            "onset_candidates": _onset_candidates(
                energy_values, ENERGY_WINDOW_SECONDS
            ),
