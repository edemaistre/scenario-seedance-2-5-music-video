...
def make_manifest(master: Path) -> dict[str, object]:
    return {
        ...
        "delivery": {
            ...
            "audio_policy": {"source": "supplied_master", "start_seconds": "0.000"},
        },
    }
...
class VerifyDeliveryTests(unittest.TestCase):
    ...
    def test_reports_transcode_as_non_bit_exact_while_preserving_timing(self) -> None:
        ...
        wav_master = self.fixture_dir / "lossless master.wav"
        run(
            [
                "ffmpeg", "-v", "error", "-f", "lavfi", "-i",
                "sine=frequency=660:sample_rate=48000:duration=30",
                "-ac", "2", "-c:a", "pcm_s16le", "-y", str(wav_master),
            ]
        )
        transcoded = self.fixture_dir / "aac transcode final.mp4"
        self._make_delivery(transcoded, audio_input=wav_master, audio_codec="aac")
