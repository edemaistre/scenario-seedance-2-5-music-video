...
class PackageSkillTests(unittest.TestCase):
    ...
    def test_package_audit_rejects_utf8_decodable_binary_signature(self) -> None:
        ...
        private_job = audit_release_file(Path("notes.md"), ("job_" + "b" * 24).encode())
        assigned_key = audit_release_file(
            Path("notes.md"), ("SCENARIO_" + "API_KEY=" + "c" * 32).encode()
        )
        embedded_audio = audit_release_file(
            Path("notes.md"), ("data:" + "audio/wav;base64," + "d" * 40).encode()
        )
        basic = audit_release_file(
            Path("notes.md"), ("Authorization: " + "Basic " + "Z" * 44).encode()
        )
        json_secret = audit_release_file(
            Path("notes.md"), ("{\"api_" + "secret\": \"" + "e" * 40 + "\"}").encode()
        )
        ...
        self.assertTrue(any("secret assignment" in item for item in assigned_key))
        self.assertTrue(any("embedded media" in item for item in embedded_audio))
