...
from scripts.verify_delivery import preferred_temp_root, verify_delivery
...
class VerifyDeliveryTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.fixture_dir = Path(
            tempfile.mkdtemp(prefix="scenario-seedance-delivery-tests-", dir=preferred_temp_root())
        )
    ...
    def test_rejects_matching_quicktime_mov_container(self) -> None:
        """Break caught: accepting matching streams delivered in a non-MP4 container."""
        mov_delivery = self.fixture_dir / "matching quicktime container.mov"
        self._make_delivery(mov_delivery, audio_input=self.master, audio_codec="copy")

        result = verify_delivery(mov_delivery, self.master, self.manifest)

        self.assertFalse(result["passed"])
        self.assertFalse(result["checks"]["mp4_container"])
        self.assertEqual(result["technical"]["container"]["major_brand"], "qt  ")
