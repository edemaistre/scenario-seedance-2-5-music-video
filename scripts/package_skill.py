...
PRIVATE_JOB_PATTERN = re.compile(r"\bjob_[A-Za-z0-9_-]{16,}\b", re.IGNORECASE)
...
SECRET_NAME_PATTERN = (
    r"(?:(?:SCENARIO|OPENAI|GITHUB|AWS)[_-])?"
    r"(?:API[_-]?(?:KEY|SECRET)|ACCESS[_-]?(?:KEY|TOKEN)|"
    r"SECRET[_-]?KEY|AUTH[_-]?TOKEN|PASSWORD)"
)
SECRET_ASSIGNMENT_PATTERN = re.compile(
    rf"(?<![A-Za-z0-9_-])['\"]?{SECRET_NAME_PATTERN}['\"]?"
...
BASIC_AUTH_PATTERN = re.compile(
    r"\bAuthorization\s*:\s*Basic\s+[A-Za-z0-9+/]{16,}={0,2}",
    re.IGNORECASE,
)
...
EMBEDDED_MEDIA_PATTERN = re.compile(
    r"data:(?:image|audio|video)/[A-Za-z0-9.+-]+;base64,[A-Za-z0-9+/=]{16,}",
    re.IGNORECASE,
)
SIGNED_URL_PATTERN = re.compile(
    r"(?:X-(?:Amz|Goog)-(?:Algorithm|Credential|Date|Expires|SignedHeaders|Signature)"
    r"|[?&](?:signature|sig|token|policy|key-pair-id)="
    r"|[?&]hdnts=[^\s#&]{1,512}~hmac=[A-Fa-f0-9]{16,})",
    re.IGNORECASE,
)
...
def audit_release_file(relative_path: Path, raw: bytes) -> list[str]:
    ...
    if PRIVATE_JOB_PATTERN.search(text):
        violations.append("private job ID found")
    if CREDENTIAL_PATTERN.search(text):
        violations.append("credential token found")
    if SECRET_ASSIGNMENT_PATTERN.search(text):
        violations.append("secret assignment found")
    if BEARER_TOKEN_PATTERN.search(text):
        violations.append("bearer authorization token found")
    if BASIC_AUTH_PATTERN.search(text):
        violations.append("basic authorization credential found")
    if JWT_PATTERN.search(text):
        violations.append("JWT token found")
    if PRIVATE_KEY_PATTERN.search(text):
        violations.append("private key block found")
    if EMBEDDED_MEDIA_PATTERN.search(text):
        violations.append("embedded media data URI found")
